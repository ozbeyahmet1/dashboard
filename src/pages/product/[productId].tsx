...
const ProductDetails = ({
  ...
  const twClasses = {
    ...
    tagXl: "w-fit rounded-md bg-baseColor bg-opacity-60 p-2 text-base lg:text-xl uppercase text-white md:p-3 lg:p-5",
  };
        ...
        <section className="w-full border-b-[2px] border-dashed border-white">
          <div className="mb-8">
            ...
            <div className="flex grid-cols-2 gap-2">
              {categories.map((category, i) => {
                return (
                  <p key={i} className={twClasses.tagXl}>
                    {category.name}
                  </p>
                );
              })}
          ...
          <div className="mb-8 flex flex-col items-start gap-6 lg:flex-row lg:items-end">
            <div>
              ...
              <div className="grid grid-cols-2 grid-rows-2 gap-2 gap-x-2 gap-y-2 md:flex">
                {businessModels.map((businessModel, i) => {
                  return (
                    <p className={twClasses.tagSm} key={i}>
                      {businessModel.name}
                    </p>
                  );
                })}
            ...
            <div>
              ...
              <p className={twClasses.tagSm}>{investmentEffort}</p>
            </div>
          </div>
          <div className="mb-8">
            <h3 className={twClasses.headers}>TRL</h3>
            <p className={twClasses.tagSm}>{trl.name}</p>
          </div>
        </section>

        {/* Video */}
        <section className="w-full border-b-[2px] border-dashed border-white py-8">
